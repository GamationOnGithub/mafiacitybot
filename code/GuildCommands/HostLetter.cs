    ...
    public static class HostLetter {
        public static async Task CreateCommand(DiscordSocketClient client, SocketGuild? guild = null)
        {
            ...
            command.AddOption(new SlashCommandOptionBuilder()
                ...
                .WithType(ApplicationCommandOptionType.SubCommand)
                .AddOption("letter", ApplicationCommandOptionType.Integer, "The *Letter* number to view.", isRequired: false)
            ).AddOption(new SlashCommandOptionBuilder()
                .WithName("remove")
                ...
                .WithType(ApplicationCommandOptionType.SubCommand)
                .AddOption("letter", ApplicationCommandOptionType.Integer, "The *Letter* number to delete.", isRequired: true)
                .AddOption("recipient", ApplicationCommandOptionType.User, "The PLAYER you wish to send the *Letter* to.", isRequired: true)
            ).AddOption(new SlashCommandOptionBuilder()
                .WithName("limit")
                .WithDescription("Set letter limit of a player.")
                .WithType(ApplicationCommandOptionType.SubCommand)
                .AddOption("player", ApplicationCommandOptionType.User, "The PLAYER to change the per-*Phase* limit for.", isRequired: true)
                .AddOption("limit", ApplicationCommandOptionType.Number, "The new per-phase *Letter* limit.", isRequired: true)
            ).AddOption(new SlashCommandOptionBuilder()
                .WithName("send")
